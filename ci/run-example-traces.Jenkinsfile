        ...
        node('docker') {
          stage(name) {
            ...
            docker.image('ctsrd/testrig-exampletraces').inside {
              ...
              def retval = sh returnStatus: true, script: "cd ${name} && mkdir failures && /home/jenkins/TestRIG/utils/scripts/runTestRIG.py -a ${conf[0]} -b ${conf[1]} -r ${conf[2]} --verification-archstring ${conf[3]} -S failures/ -d /home/jenkins/TestRIG_exampleTraces/${conf[4]}"
