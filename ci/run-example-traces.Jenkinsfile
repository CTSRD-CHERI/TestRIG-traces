  ...
  conf ->
    ...
    jobs.put(name, {
      ansiColor('xterm') {
        node('docker') {
          stage(name) {
            docker.image('ctsrd/testrig-traces').pull()
            docker.image('ctsrd/testrig-traces').inside {
              echo name
              sh "rm -rf ${name} && mkdir ${name}"
              sh "mkdir ${name}/tests && find /home/jenkins/TestRIG-traces/ -type f | grep -xvf /home/jenkins/TestRIG-traces/ci/implementation-independent.ignore | grep -xvf /home/jenkins/TestRIG-traces/ci/${name}.ignore | xargs cp -t ${name}/tests --parents"
